...
import { INT, DOUBLE, STRING, TAG_SIZE, TAG_MASK } from './constants.js'
import { createSeekPath } from './seekers.js'
...
function compare(buffer1, start1, buffer2, start2) {
  ...
  const tag1 = fvdecode(buffer1, start1)
  const len1 = fvdecode.bytes
  const tag2 = fvdecode(buffer2, start2)
  const len2 = fvdecode.bytes
