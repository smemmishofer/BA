import {fvdecode, fvencode, fvencodingLength} from "./fast-varint.js";
import { TAG_SIZE, TAG_MASK } from './constants.js'
import {
  STRING,
  BUFFER,
  INT,
...
function encodingLength(value) {
  ...
  return fvencodingLength(len << TAG_SIZE) + len
}
...
function encode(value, buffer, start, _len) {
  ...
  fvencode((len << TAG_SIZE) | type, buffer, start)
  const bytes = fvencode.bytes
...
function getEncodedLength(buffer, start) {
  return fvdecode(buffer, start) >> TAG_SIZE
}

function getEncodedType(buffer, start) {
  return fvdecode(buffer, start) & TAG_MASK
}
